...
class AtomTypeClassifier:
    ...
    def apply_to_molecule(self,molecule):
        for iatm,atm in enumerate(molecule.atms):
            ...
            #print(iatm, atm.name, aclass)
            #check input <-> class hyb state consistency
            if CORRECT_HYBRID[aclass] >0 and atm.hyb != CORRECT_HYBRID[aclass] and atm.hyb != 8:
                molecule.atms[iatm].hyb = CORRECT_HYBRID[aclass]
...
class FunctionalGroupClassifier:
    ...
    def apply_to_molecule(self,molecule):
        ...
        for ring in molecule.rings:
            ...
            if ring.type in [1,2]:
                molecule.funcgrps.append(FunctionalGroupClass(atms,'Ring-pucker'))
                funcgrp_assigned += ring.atms
            elif ring.type == 4:
                molecule.funcgrps.append(FunctionalGroupClass(atms,'Ring-aro'))
                funcgrp_assigned += ring.atms
