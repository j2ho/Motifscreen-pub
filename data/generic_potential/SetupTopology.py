...
def setup(mol,option): # mol: Molecule type
    ...
    if option.opt.nbratom != None:
        atmnames = [atm.name for atm in mol.atms]
        try:
            mol.nbratom = atmnames.index(option.opt.nbratom)  #override
        except:
            print("failed to find atom name %s for option.nbratom; take default instead"%option.opt.nbratm)
    ...
    # reassign ring after atom type correction
    detect_rings(mol,option)
...
def classify_ring_type(mol,ring,option):
    ...
    if ring.natms > 6: 
        ...
        for atm in ring.atms:
            ...
            elif len(ring.atms) == 6 and nsp2 == 6:
                is_aro = True
