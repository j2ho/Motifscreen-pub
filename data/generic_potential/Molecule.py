...
class MoleculeClass:
    def __init__(self,mol2file, option=None, mol2fileobj=None):
        ...
        if option == None:
            if mol2file == None:
                self.option = OptionClass(['','-s','dumm'])
            else:
                self.option = OptionClass(sys.argv)
        else:
            self.option = option
    ...
    def initialize_by_info(self,elems,hybs,bonds):
        atms = []
        counts = {}
        xyzs = []
        
        for elem,hyb in zip(elems,hybs):
            if elem not in counts:
                counts[elem] = 0
            counts[elem] += 1
            atype = ATYPES.index(elem)
            name = elem + str(counts[elem])
            
            atms.append( AtomClass(name,atype,hyb,0.0) )
            xyzs.append([0.0,0.0,0.0])
        
        stat = self.clean_info(atms, bonds, xyzs)
        SetupTopology.setup(self,option=self.option)
    ...
    def read_mol2(self):
        ...
        for l in mol2filelines:
            ...
            if l.startswith('@<TRIPOS>MOLECULE'):
                ...
                mode = 2
                continue
            elif l.startswith('@<TRIPOS>BOND') or l.startswith('@<TRIPOS>UNITY_ATOM_ATTR'):
                mode = 3
                continue
            ...
            if mode == 2:
                if len(words) < 6:
                    raise Exception("mol2 format is invalid, probably missing info!")
    ...
    def clean_info(self, atms, bonds, xyzs):
        newindex = [(atm.atype==2,i) for i,atm in enumerate(atms)]
